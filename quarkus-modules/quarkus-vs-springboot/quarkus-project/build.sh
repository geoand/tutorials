...
if [ "$1" = "native" ]; then
    mvn clean package -Pnative -Dquarkus.container-image.build=true -Dquarkus.native.container-build=true -f $SCRIPTPATH/pom.xml &&
    ...
    mvn clean package -Dquarkus.container-image.build=true -f $SCRIPTPATH/pom.xml &&
